...
int NinjaMain::ToolGraph(const Options* options, int argc, char* argv[]) {
  ...
  Warning("targets = %d", db->treenodes_.size());
  // for(int i=0;i< db->treenodes_.size();i++){
  // char size[1024] = "";
  // //char height[1024] = "";
  // sprintf(size,"%s",db->treenodes_[i]->valuedot_.fontsize);
  // sprintf(height,"%s",db->treenodes_[i]->valuedot_.height);
  // 转 height 有一个 段错误

  //   Warning(string(db->treenodes_[i]->name_ + "[label=" + db->treenodes_[i]->valuedot_.label_ + ", color="+ db->treenodes_[i]->valuedot_.color+ \
  //   ", shape="+ db->treenodes_[i]->valuedot_.shape +  /* ",fontsize="  + string(size) + */ ", height = "  /* +  string(height) */  +"]" ).c_str());

  //   // print father
  //   for(auto iter =
  //   db->treenodes_[i]->dict_father_.begin();iter!=db->treenodes_[i]->dict_father_.end();iter++){
  //     Warning( string(db->treenodes_[i]->name_ + " -> " +  iter->first +
  //     "[arrowhead="+ iter->second.arrowhead_ +",
  //     shape="+iter->second.style_+
...
int ReadFlags(int* argc, char*** argv,
  ...
  for (int i = 0; i < *argc; i++) {
    if (0 == strcmp("-ignore", (*argv)[i])) {
      flag_ignore = 1;
      *argc -= 1;
      break;
    }
    if (0 != strcmp("-ignore", (*argv)[i]) && (*argv)[i][0] == '-' &&
        (*argv)[i][1] == 'i') {
      *argc -= 1;
       Error("Usage: -ignore to drop g.android.xxx informations of dot");
    }
  }
